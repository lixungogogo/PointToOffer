...
********************************************/
template<class T, int N = 10>
size_t DStack<T, N>::Size()const
{
	return _size;
}
template<class T, int N = 10>
T& DStack<T, N>::operator[](size_t index)
{
	if (index >= Size() || Empty())
		return -1;
	return _data[index];
...
template<class T, int N = 10>
const T& DStack<T, N>::operator[](size_t index)const
{
	if (index >= Size() || Empty())
		return -1;
	return _data[index];
...
template<class T, int N = 10>
void DStack<T, N>::Print()const
{
	size_t iCount = 0;
	while (iCount < Size())
	{
		cout << _data[iCount++] << endl;
	}
}